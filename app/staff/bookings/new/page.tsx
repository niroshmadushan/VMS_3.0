...
export default function StaffNewBookingPage() {
  ...
  return (
    <RouteProtection requiredRole="staff">
      <DashboardLayout title="Create New Booking" subtitle="Fill in the details to create a new booking">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Responsible Person Section - Read Only for Staff */}
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              You are automatically set as the responsible person for this booking
            </p>
          </CardHeader>
          <CardContent>
                ...
                <div className="flex items-center gap-3">
                  <Avatar className="h-12 w-12 border-2 border-primary">
                    <AvatarFallback className="bg-primary text-primary-foreground font-semibold text-base">
                      {formData.responsiblePerson.name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="font-semibold text-base">{formData.responsiblePerson.name}</p>
                      <Lock className="h-4 w-4 text-muted-foreground" />
                    </div>
                    <p className="text-sm text-muted-foreground">{formData.responsiblePerson.email}</p>
                    <Badge variant="outline" className="mt-2 text-xs">{formData.responsiblePerson.role}</Badge>
                  </div>
                </div>
              </div>
            ) : (
              <div className="p-4 bg-muted/50 border-2 border-dashed rounded-lg text-center">
